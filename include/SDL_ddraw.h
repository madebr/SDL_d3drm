...
SDL_DEFINE_GUID( SDL_CLSID_DirectDraw,                0xD7B70EE0,0x4340,0x11CF,0xB0,0x63,0x00,0x20,0xAF,0xC2,0xCD,0x35 );
SDL_DEFINE_GUID( SDL_CLSID_DirectDrawClipper,         0x593817A0,0x7DB3,0x11CF,0xA2,0xDE,0x00,0xAA,0x00,0xb9,0x33,0x56 );
SDL_DEFINE_GUID( SDL_IID_IDirectDraw,                 0x6C14DB80,0xA733,0x11CE,0xA5,0x21,0x00,0x20,0xAF,0x0B,0xE5,0x60 );
SDL_DEFINE_GUID( SDL_IID_IDirectDraw2,                0xB3A6F3E0,0x2B43,0x11CF,0xA2,0xDE,0x00,0xAA,0x00,0xB9,0x33,0x56 );
SDL_DEFINE_GUID( SDL_IID_IDirectDrawSurface,          0x6C14DB81,0xA733,0x11CE,0xA5,0x21,0x00,0x20,0xAF,0x0B,0xE5,0x60 );
SDL_DEFINE_GUID( SDL_IID_IDirectDrawSurface2,         0x57805885,0x6eec,0x11cf,0x94,0x41,0xa8,0x23,0x03,0xc1,0x0e,0x27 );
SDL_DEFINE_GUID( SDL_IID_IDirectDrawSurface3,         0xDA044E00,0x69B2,0x11D0,0xA1,0xD5,0x00,0xAA,0x00,0xB8,0xDF,0xBB )    ;

SDL_DEFINE_GUID( SDL_IID_IDirectDrawPalette,          0x6C14DB84,0xA733,0x11CE,0xA5,0x21,0x00,0x20,0xAF,0x0B,0xE5,0x60 );
SDL_DEFINE_GUID( SDL_IID_IDirectDrawClipper,          0x6C14DB85,0xA733,0x11CE,0xA5,0x21,0x00,0x20,0xAF,0x0B,0xE5,0x60 );
SDL_DEFINE_GUID( SDL_IID_IDirectDrawColorControl,     0x4B9F0EE0,0x0D7E,0x11D0,0x9B,0x06,0x00,0xA0,0xC9,0x03,0xA3,0xB8 );
...
typedef IDirectDraw              SDL_FAR *LPDIRECTDRAW;
typedef IDirectDraw2             SDL_FAR *LPDIRECTDRAW2;
typedef IDirectDrawSurface       SDL_FAR *LPDIRECTDRAWSURFACE;
typedef IDirectDrawSurface2      SDL_FAR *LPDIRECTDRAWSURFACE2;
typedef IDirectDrawSurface3      SDL_FAR *LPDIRECTDRAWSURFACE3;

typedef struct IDirectDrawPalette       SDL_FAR *LPDIRECTDRAWPALETTE;
typedef struct IDirectDrawClipper       SDL_FAR *LPDIRECTDRAWCLIPPER;
typedef struct IDirectDrawColorControl  SDL_FAR *LPDIRECTDRAWCOLORCONTROL;

typedef struct _DDFXROP                 SDL_FAR *LPDDFXROP;
typedef struct _DDSURFACEDESC           SDL_FAR *LPDDSURFACEDESC;
typedef struct _DDCOLORCONTROL          SDL_FAR *LPDDCOLORCONTROL;
...
typedef SDL_BOOL (SDL_FAR SDL_PASCAL * LPDDENUMCALLBACKA)(SDL_GUID SDL_FAR *, SDL_LPSTR, SDL_LPSTR, SDL_LPVOID);
typedef SDL_BOOL (SDL_FAR SDL_PASCAL * LPDDENUMCALLBACKW)(SDL_GUID SDL_FAR *, SDL_LPWSTR, SDL_LPWSTR, SDL_LPVOID);
extern SDL_HRESULT SDL_WINAPI DirectDrawEnumerateW( LPDDENUMCALLBACKW lpCallback, SDL_LPVOID lpContext );
extern SDL_HRESULT SDL_WINAPI DirectDrawEnumerateA( LPDDENUMCALLBACKA lpCallback, SDL_LPVOID lpContext );
#ifdef UNICODE
...
#endif
extern SDL_HRESULT SDL_WINAPI DirectDrawCreate( SDL_GUID SDL_FAR *lpGUID, LPDIRECTDRAW SDL_FAR *lplpDD, SDL_IUnknown SDL_FAR *pUnkOuter );
extern SDL_HRESULT SDL_WINAPI DirectDrawCreateClipper( SDL_DWORD dwFlags, LPDIRECTDRAWCLIPPER SDL_FAR *lplpDDClipper, SDL_IUnknown SDL_FAR *pUnkOuter );
#endif
...
//#ifndef WINNT
typedef SDL_HRESULT (SDL_FAR SDL_PASCAL * LPDDENUMMODESCALLBACK)(LPDDSURFACEDESC, SDL_LPVOID);
typedef SDL_HRESULT (SDL_FAR SDL_PASCAL * LPDDENUMSURFACESCALLBACK)(LPDIRECTDRAWSURFACE, LPDDSURFACEDESC, SDL_LPVOID);
//#endif
/*
 * DDCOLORKEY
 */
...
{
    SDL_DWORD	dwColorSpaceLowValue;	// low boundary of color space that is to
    // be treated as Color Key, inclusive
    SDL_DWORD	dwColorSpaceHighValue;	// high boundary of color space that is
    // to be treated as Color Key, inclusive
} DDCOLORKEY;

typedef DDCOLORKEY SDL_FAR* LPDDCOLORKEY;
...
{
    SDL_DWORD	dwSize;				// size of structure
    SDL_DWORD	dwDDFX;				// FX operations
    SDL_DWORD	dwROP;				// Win32 raster operations
    SDL_DWORD	dwDDROP;			// Raster operations new for DirectDraw
    SDL_DWORD	dwRotationAngle;		// Rotation angle for blt
    SDL_DWORD	dwZBufferOpCode;		// ZBuffer compares
    SDL_DWORD	dwZBufferLow;			// Low limit of Z buffer
    SDL_DWORD	dwZBufferHigh;			// High limit of Z buffer
    SDL_DWORD	dwZBufferBaseDest;		// Destination base value
    SDL_DWORD	dwZDestConstBitDepth;		// Bit depth used to specify Z constant for destination
    union
    {
        SDL_DWORD	dwZDestConst;			// Constant to use as Z buffer for dest
        LPDIRECTDRAWSURFACE lpDDSZBufferDest;	// Surface to use as Z buffer for dest
    };
    SDL_DWORD	dwZSrcConstBitDepth;		// Bit depth used to specify Z constant for source
    union
    {
        SDL_DWORD	dwZSrcConst;			// Constant to use as Z buffer for src
        LPDIRECTDRAWSURFACE lpDDSZBufferSrc;	// Surface to use as Z buffer for src
    };
    SDL_DWORD	dwAlphaEdgeBlendBitDepth;	// Bit depth used to specify constant for alpha edge blend
    SDL_DWORD	dwAlphaEdgeBlend;		// Alpha for edge blending
    SDL_DWORD	dwReserved;
    SDL_DWORD	dwAlphaDestConstBitDepth;	// Bit depth used to specify alpha constant for destination
    union
    {
        SDL_DWORD	dwAlphaDestConst;		// Constant to use as Alpha Channel
        LPDIRECTDRAWSURFACE lpDDSAlphaDest;	// Surface to use as Alpha Channel
    };
    SDL_DWORD	dwAlphaSrcConstBitDepth;	// Bit depth used to specify alpha constant for source
    union
    {
        SDL_DWORD	dwAlphaSrcConst;		// Constant to use as Alpha Channel
        LPDIRECTDRAWSURFACE lpDDSAlphaSrc;	// Surface to use as Alpha Channel
    };
    union
    {
        SDL_DWORD	dwFillColor;			// color in RGB or Palettized
        SDL_DWORD   dwFillDepth;                    // depth value for z-buffer
        SDL_DWORD	dwFillPixel;			// pixel value for RGBA or RGBZ
...
typedef DDBLTFX SDL_FAR* LPDDBLTFX;
...
{
    SDL_DWORD	dwCaps;		// capabilities of surface wanted
} DDSCAPS;

typedef DDSCAPS SDL_FAR* LPDDSCAPS;
...
{
    SDL_DWORD	dwSize;			// size of the DDDRIVERCAPS structure
    SDL_DWORD	dwCaps;			// driver specific capabilities
    SDL_DWORD	dwCaps2;		// more driver specific capabilites
    SDL_DWORD	dwCKeyCaps;		// color key capabilities of the surface
    SDL_DWORD	dwFXCaps;		// driver specific stretching and effects capabilites
    SDL_DWORD	dwFXAlphaCaps;		// alpha driver specific capabilities
    SDL_DWORD	dwPalCaps;		// palette capabilities
    SDL_DWORD	dwSVCaps;		// stereo vision capabilities
    SDL_DWORD	dwAlphaBltConstBitDepths;	// DDBD_2,4,8
    SDL_DWORD	dwAlphaBltPixelBitDepths;	// DDBD_1,2,4,8
    SDL_DWORD	dwAlphaBltSurfaceBitDepths;	// DDBD_1,2,4,8
    SDL_DWORD	dwAlphaOverlayConstBitDepths;	// DDBD_2,4,8
    SDL_DWORD	dwAlphaOverlayPixelBitDepths;	// DDBD_1,2,4,8
    SDL_DWORD	dwAlphaOverlaySurfaceBitDepths; // DDBD_1,2,4,8
    SDL_DWORD	dwZBufferBitDepths;		// DDBD_8,16,24,32
    SDL_DWORD	dwVidMemTotal;		// total amount of video memory
    SDL_DWORD	dwVidMemFree;		// amount of free video memory
    SDL_DWORD	dwMaxVisibleOverlays;	// maximum number of visible overlays
    SDL_DWORD	dwCurrVisibleOverlays;	// current number of visible overlays
    SDL_DWORD	dwNumFourCCCodes;	// number of four cc codes
    SDL_DWORD	dwAlignBoundarySrc;	// source rectangle alignment
    SDL_DWORD	dwAlignSizeSrc;		// source rectangle byte size
    SDL_DWORD	dwAlignBoundaryDest;	// dest rectangle alignment
    SDL_DWORD	dwAlignSizeDest;	// dest rectangle byte size
    SDL_DWORD	dwAlignStrideAlign;	// stride alignment
    SDL_DWORD	dwRops[DD_ROP_SPACE];	// ROPS supported
    DDSCAPS	ddsCaps;		// DDSCAPS structure has all the general capabilities
    SDL_DWORD	dwMinOverlayStretch;	// minimum overlay stretch factor multiplied by 1000, eg 1000 == 1.0, 1300 == 1.3
    SDL_DWORD	dwMaxOverlayStretch;	// maximum overlay stretch factor multiplied by 1000, eg 1000 == 1.0, 1300 == 1.3
    SDL_DWORD	dwMinLiveVideoStretch;	// minimum live video stretch factor multiplied by 1000, eg 1000 == 1.0, 1300 == 1.3
    SDL_DWORD	dwMaxLiveVideoStretch;	// maximum live video stretch factor multiplied by 1000, eg 1000 == 1.0, 1300 == 1.3
    SDL_DWORD	dwMinHwCodecStretch;	// minimum hardware codec stretch factor multiplied by 1000, eg 1000 == 1.0, 1300 == 1.3
    SDL_DWORD	dwMaxHwCodecStretch;	// maximum hardware codec stretch factor multiplied by 1000, eg 1000 == 1.0, 1300 == 1.3
    SDL_DWORD	dwReserved1;		// reserved
    SDL_DWORD	dwReserved2;		// reserved
    SDL_DWORD	dwReserved3;		// reserved
    SDL_DWORD	dwSVBCaps;		// driver specific capabilities for System->Vmem blts
    SDL_DWORD	dwSVBCKeyCaps;		// driver color key capabilities for System->Vmem blts
    SDL_DWORD	dwSVBFXCaps;		// driver FX capabilities for System->Vmem blts
    SDL_DWORD	dwSVBRops[DD_ROP_SPACE];// ROPS supported for System->Vmem blts
    SDL_DWORD	dwVSBCaps;		// driver specific capabilities for Vmem->System blts
    SDL_DWORD	dwVSBCKeyCaps;		// driver color key capabilities for Vmem->System blts
    SDL_DWORD	dwVSBFXCaps;		// driver FX capabilities for Vmem->System blts
    SDL_DWORD	dwVSBRops[DD_ROP_SPACE];// ROPS supported for Vmem->System blts
    SDL_DWORD	dwSSBCaps;		// driver specific capabilities for System->System blts
    SDL_DWORD	dwSSBCKeyCaps;		// driver color key capabilities for System->System blts
    SDL_DWORD	dwSSBFXCaps;		// driver FX capabilities for System->System blts
    SDL_DWORD	dwSSBRops[DD_ROP_SPACE];// ROPS supported for System->System blts
    SDL_DWORD	dwReserved4;		// reserved
    SDL_DWORD	dwReserved5;		// reserved
    SDL_DWORD	dwReserved6;		// reserved
} DDCAPS_DX3;
typedef DDCAPS_DX3 SDL_FAR* LPDDCAPS_DX3;
#endif /* DIRECTDRAW_VERSION >= 0x0500 */

typedef struct _DDCAPS
{
/*  0*/ SDL_DWORD	dwSize;			// size of the DDDRIVERCAPS structure
/*  4*/ SDL_DWORD	dwCaps;			// driver specific capabilities
/*  8*/ SDL_DWORD	dwCaps2;		// more driver specific capabilites
/*  c*/ SDL_DWORD	dwCKeyCaps;		// color key capabilities of the surface
/* 10*/ SDL_DWORD	dwFXCaps;		// driver specific stretching and effects capabilites
/* 14*/ SDL_DWORD	dwFXAlphaCaps;		// alpha driver specific capabilities
/* 18*/ SDL_DWORD	dwPalCaps;		// palette capabilities
/* 1c*/ SDL_DWORD	dwSVCaps;		// stereo vision capabilities
/* 20*/ SDL_DWORD	dwAlphaBltConstBitDepths;	// DDBD_2,4,8
/* 24*/ SDL_DWORD	dwAlphaBltPixelBitDepths;	// DDBD_1,2,4,8
/* 28*/ SDL_DWORD	dwAlphaBltSurfaceBitDepths;	// DDBD_1,2,4,8
/* 2c*/ SDL_DWORD	dwAlphaOverlayConstBitDepths;	// DDBD_2,4,8
/* 30*/ SDL_DWORD	dwAlphaOverlayPixelBitDepths;	// DDBD_1,2,4,8
/* 34*/ SDL_DWORD	dwAlphaOverlaySurfaceBitDepths; // DDBD_1,2,4,8
/* 38*/ SDL_DWORD	dwZBufferBitDepths;		// DDBD_8,16,24,32
/* 3c*/ SDL_DWORD	dwVidMemTotal;		// total amount of video memory
/* 40*/ SDL_DWORD	dwVidMemFree;		// amount of free video memory
/* 44*/ SDL_DWORD	dwMaxVisibleOverlays;	// maximum number of visible overlays
/* 48*/ SDL_DWORD	dwCurrVisibleOverlays;	// current number of visible overlays
/* 4c*/ SDL_DWORD	dwNumFourCCCodes;	// number of four cc codes
/* 50*/ SDL_DWORD	dwAlignBoundarySrc;	// source rectangle alignment
/* 54*/ SDL_DWORD	dwAlignSizeSrc;		// source rectangle byte size
/* 58*/ SDL_DWORD	dwAlignBoundaryDest;	// dest rectangle alignment
/* 5c*/ SDL_DWORD	dwAlignSizeDest;	// dest rectangle byte size
/* 60*/ SDL_DWORD	dwAlignStrideAlign;	// stride alignment
/* 64*/ SDL_DWORD	dwRops[DD_ROP_SPACE];	// ROPS supported
/* 84*/ DDSCAPS	ddsCaps;		// DDSCAPS structure has all the general capabilities
/* 88*/ SDL_DWORD	dwMinOverlayStretch;	// minimum overlay stretch factor multiplied by 1000, eg 1000 == 1.0, 1300 == 1.3
/* 8c*/ SDL_DWORD	dwMaxOverlayStretch;	// maximum overlay stretch factor multiplied by 1000, eg 1000 == 1.0, 1300 == 1.3
/* 90*/ SDL_DWORD	dwMinLiveVideoStretch;	// minimum live video stretch factor multiplied by 1000, eg 1000 == 1.0, 1300 == 1.3
/* 94*/ SDL_DWORD	dwMaxLiveVideoStretch;	// maximum live video stretch factor multiplied by 1000, eg 1000 == 1.0, 1300 == 1.3
/* 98*/ SDL_DWORD	dwMinHwCodecStretch;	// minimum hardware codec stretch factor multiplied by 1000, eg 1000 == 1.0, 1300 == 1.3
/* 9c*/ SDL_DWORD	dwMaxHwCodecStretch;	// maximum hardware codec stretch factor multiplied by 1000, eg 1000 == 1.0, 1300 == 1.3
/* a0*/ SDL_DWORD	dwReserved1;		// reserved
/* a4*/ SDL_DWORD	dwReserved2;		// reserved
/* a8*/ SDL_DWORD	dwReserved3;		// reserved
/* ac*/ SDL_DWORD	dwSVBCaps;		// driver specific capabilities for System->Vmem blts
/* b0*/ SDL_DWORD	dwSVBCKeyCaps;		// driver color key capabilities for System->Vmem blts
/* b4*/ SDL_DWORD	dwSVBFXCaps;		// driver FX capabilities for System->Vmem blts
/* b8*/ SDL_DWORD	dwSVBRops[DD_ROP_SPACE];// ROPS supported for System->Vmem blts
/* d8*/ SDL_DWORD	dwVSBCaps;		// driver specific capabilities for Vmem->System blts
/* dc*/ SDL_DWORD	dwVSBCKeyCaps;		// driver color key capabilities for Vmem->System blts
/* e0*/ SDL_DWORD	dwVSBFXCaps;		// driver FX capabilities for Vmem->System blts
/* e4*/ SDL_DWORD	dwVSBRops[DD_ROP_SPACE];// ROPS supported for Vmem->System blts
/*104*/ SDL_DWORD	dwSSBCaps;		// driver specific capabilities for System->System blts
/*108*/ SDL_DWORD	dwSSBCKeyCaps;		// driver color key capabilities for System->System blts
/*10c*/ SDL_DWORD	dwSSBFXCaps;		// driver FX capabilities for System->System blts
/*110*/ SDL_DWORD	dwSSBRops[DD_ROP_SPACE];// ROPS supported for System->System blts
#if       DIRECTDRAW_VERSION >= 0x0500
    /*130*/ SDL_DWORD	dwMaxVideoPorts;	// maximum number of usable video ports
/*134*/ SDL_DWORD	dwCurrVideoPorts;	// current number of video ports used
/*138*/ SDL_DWORD	dwSVBCaps2;		// more driver specific capabilities for System->Vmem blts
/*13c*/ SDL_DWORD	dwNLVBCaps;		  // driver specific capabilities for non-local->local vidmem blts
/*140*/ SDL_DWORD	dwNLVBCaps2;		  // more driver specific capabilities non-local->local vidmem blts
/*144*/ SDL_DWORD	dwNLVBCKeyCaps;		  // driver color key capabilities for non-local->local vidmem blts
/*148*/ SDL_DWORD	dwNLVBFXCaps;		  // driver FX capabilities for non-local->local blts
/*14c*/ SDL_DWORD	dwNLVBRops[DD_ROP_SPACE]; // ROPS supported for non-local->local blts
#else  /* DIRECTDRAW_VERSION >= 0x0500 */
/*130*/ SDL_DWORD	dwReserved4;		// reserved
/*134*/ SDL_DWORD	dwReserved5;		// reserved
/*138*/ SDL_DWORD	dwReserved6;		// reserved
#endif /* DIRECTDRAW_VERSION >= 0x0500 */
} DDCAPS;

typedef DDCAPS SDL_FAR* LPDDCAPS;
...
{
    SDL_DWORD	dwSize;			// size of structure
    SDL_DWORD	dwFlags;		// pixel format flags
    SDL_DWORD	dwFourCC;		// (FOURCC code)
    union
    {
        SDL_DWORD	dwRGBBitCount;		// how many bits per pixel
        SDL_DWORD	dwYUVBitCount;		// how many bits per pixel
        SDL_DWORD	dwZBufferBitDepth;	// how many bits for z buffers
        SDL_DWORD	dwAlphaBitDepth;	// how many bits for alpha channels
    };
    union
    {
        SDL_DWORD	dwRBitMask;		// mask for red bit
        SDL_DWORD	dwYBitMask;		// mask for Y bits
    };
    union
    {
        SDL_DWORD	dwGBitMask;		// mask for green bits
        SDL_DWORD	dwUBitMask;		// mask for U bits
    };
    union
    {
        SDL_DWORD	dwBBitMask;		// mask for blue bits
        SDL_DWORD	dwVBitMask;		// mask for V bits
    };
    union
    {
        SDL_DWORD	dwRGBAlphaBitMask;	// mask for alpha channel
        SDL_DWORD	dwYUVAlphaBitMask;	// mask for alpha channel
        SDL_DWORD	dwRGBZBitMask;		// mask for Z channel
        SDL_DWORD	dwYUVZBitMask;		// mask for Z channel
    };
} DDPIXELFORMAT;

typedef DDPIXELFORMAT SDL_FAR* LPDDPIXELFORMAT;
...
typedef struct _DDOVERLAYFX
{
    SDL_DWORD	dwSize;				// size of structure
    SDL_DWORD	dwAlphaEdgeBlendBitDepth;	// Bit depth used to specify constant for alpha edge blend
    SDL_DWORD	dwAlphaEdgeBlend;		// Constant to use as alpha for edge blend
    SDL_DWORD	dwReserved;
    SDL_DWORD	dwAlphaDestConstBitDepth;	// Bit depth used to specify alpha constant for destination
    union
    {
        SDL_DWORD	dwAlphaDestConst;		// Constant to use as alpha channel for dest
        LPDIRECTDRAWSURFACE lpDDSAlphaDest;	// Surface to use as alpha channel for dest
    };
    SDL_DWORD	dwAlphaSrcConstBitDepth;	// Bit depth used to specify alpha constant for source
    union
    {
        SDL_DWORD	dwAlphaSrcConst;		// Constant to use as alpha channel for src
        LPDIRECTDRAWSURFACE lpDDSAlphaSrc;	// Surface to use as alpha channel for src
    };
    DDCOLORKEY	dckDestColorkey;		// DestColorkey override
    DDCOLORKEY	dckSrcColorkey;			// DestColorkey override
    SDL_DWORD       dwDDFX;                         // Overlay FX
    SDL_DWORD	dwFlags;			// flags
} DDOVERLAYFX;

typedef DDOVERLAYFX SDL_FAR *LPDDOVERLAYFX;
...
typedef DDBLTBATCH SDL_FAR * LPDDBLTBATCH;

/*
 * callbacks
 */
typedef SDL_DWORD	(SDL_FAR SDL_PASCAL *LPCLIPPERCALLBACK)(LPDIRECTDRAWCLIPPER lpDDClipper, SDL_HWND hWnd, SDL_DWORD code, SDL_LPVOID lpContext );
#ifdef STREAMING
typedef SDL_DWORD	(SDL_FAR SDL_PASCAL *LPSURFACESTREAMINGCALLBACK)(SDL_DWORD);
#endif
...
#define SDL_INTERFACE IDirectDraw
SDL_DECLARE_INTERFACE_( IDirectDraw, SDL_IUnknown )
{
/*** IUnknown methods ***/
SDL_STDMETHOD(QueryInterface) (SDL_THIS_ SDL_REFIID riid, SDL_LPVOID SDL_FAR * ppvObj) SDL_PURE;
SDL_STDMETHOD_(SDL_ULONG,AddRef) (SDL_THIS)  SDL_PURE;
SDL_STDMETHOD_(SDL_ULONG,Release) (SDL_THIS) SDL_PURE;
/*** IDirectDraw methods ***/
SDL_STDMETHOD(Compact)(SDL_THIS) SDL_PURE;
        SDL_STDMETHOD(CreateClipper)(SDL_THIS_ SDL_DWORD, LPDIRECTDRAWCLIPPER SDL_FAR*, SDL_IUnknown SDL_FAR * ) SDL_PURE;
SDL_STDMETHOD(CreatePalette)(SDL_THIS_ SDL_DWORD, SDL_LPPALETTEENTRY, LPDIRECTDRAWPALETTE SDL_FAR*, SDL_IUnknown SDL_FAR * ) SDL_PURE;
SDL_STDMETHOD(CreateSurface)(SDL_THIS_  LPDDSURFACEDESC, LPDIRECTDRAWSURFACE SDL_FAR *, SDL_IUnknown SDL_FAR *) SDL_PURE;
SDL_STDMETHOD(DuplicateSurface)( SDL_THIS_ LPDIRECTDRAWSURFACE, LPDIRECTDRAWSURFACE SDL_FAR * ) SDL_PURE;
SDL_STDMETHOD(EnumDisplayModes)( SDL_THIS_ SDL_DWORD, LPDDSURFACEDESC, SDL_LPVOID, LPDDENUMMODESCALLBACK ) SDL_PURE;
        SDL_STDMETHOD(EnumSurfaces)(SDL_THIS_ SDL_DWORD, LPDDSURFACEDESC, SDL_LPVOID,LPDDENUMSURFACESCALLBACK ) SDL_PURE;
SDL_STDMETHOD(FlipToGDISurface)(SDL_THIS) SDL_PURE;
        SDL_STDMETHOD(GetCaps)( SDL_THIS_ LPDDCAPS, LPDDCAPS) SDL_PURE;
SDL_STDMETHOD(GetDisplayMode)( SDL_THIS_ LPDDSURFACEDESC) SDL_PURE;
        SDL_STDMETHOD(GetFourCCCodes)(SDL_THIS_  SDL_LPDWORD, SDL_LPDWORD ) SDL_PURE;
SDL_STDMETHOD(GetGDISurface)(SDL_THIS_ LPDIRECTDRAWSURFACE SDL_FAR *) SDL_PURE;
        SDL_STDMETHOD(GetMonitorFrequency)(SDL_THIS_ SDL_LPDWORD) SDL_PURE;
SDL_STDMETHOD(GetScanLine)(SDL_THIS_ SDL_LPDWORD) SDL_PURE;
        SDL_STDMETHOD(GetVerticalBlankStatus)(SDL_THIS_ SDL_LPBOOL ) SDL_PURE;
SDL_STDMETHOD(Initialize)(SDL_THIS_ SDL_GUID SDL_FAR *) SDL_PURE;
        SDL_STDMETHOD(RestoreDisplayMode)(SDL_THIS) SDL_PURE;
SDL_STDMETHOD(SetCooperativeLevel)(SDL_THIS_ SDL_HWND, SDL_DWORD) SDL_PURE;
        SDL_STDMETHOD(SetDisplayMode)(SDL_THIS_ SDL_DWORD, SDL_DWORD,SDL_DWORD) SDL_PURE;
SDL_STDMETHOD(WaitForVerticalBlank)(SDL_THIS_ SDL_DWORD, SDL_HANDLE ) SDL_PURE;
};
#undef SDL_INTERFACE
...
#if defined( _WIN32 ) && !defined( _NO_COM )
#define SDL_INTERFACE IDirectDraw2
SDL_DECLARE_INTERFACE_( IDirectDraw2, SDL_IUnknown )
{
/*** IUnknown methods ***/
SDL_STDMETHOD(QueryInterface) (SDL_THIS_ SDL_REFIID riid, SDL_LPVOID SDL_FAR * ppvObj) SDL_PURE;
SDL_STDMETHOD_(SDL_ULONG,AddRef) (SDL_THIS)  SDL_PURE;
...
SDL_STDMETHOD(Initialize)(SDL_THIS_ SDL_GUID SDL_FAR *) SDL_PURE;
        SDL_STDMETHOD(RestoreDisplayMode)(SDL_THIS) SDL_PURE;
SDL_STDMETHOD(SetCooperativeLevel)(SDL_THIS_ SDL_HWND, SDL_DWORD) SDL_PURE;
        SDL_STDMETHOD(SetDisplayMode)(SDL_THIS_ SDL_DWORD, SDL_DWORD,SDL_DWORD, SDL_DWORD, SDL_DWORD) SDL_PURE;
SDL_STDMETHOD(WaitForVerticalBlank)(SDL_THIS_ SDL_DWORD, SDL_HANDLE ) SDL_PURE;
        /*** Added in the v2 interface ***/
        SDL_STDMETHOD(GetAvailableVidMem)(SDL_THIS_ LPDDSCAPS, SDL_LPDWORD, SDL_LPDWORD) SDL_PURE;
};
#undef SDL_INTERFACE
 ...
 */
#if defined( _WIN32 ) && !defined( _NO_COM )
#define SDL_INTERFACE IDirectDrawPalette
SDL_DECLARE_INTERFACE_( IDirectDrawPalette, SDL_IUnknown )
{
/*** IUnknown methods ***/
SDL_STDMETHOD(QueryInterface) (SDL_THIS_ SDL_REFIID riid, SDL_LPVOID SDL_FAR * ppvObj) SDL_PURE;
SDL_STDMETHOD_(SDL_ULONG,AddRef) (SDL_THIS)  SDL_PURE;
...
SDL_STDMETHOD(GetCaps)(SDL_THIS_ SDL_LPDWORD) SDL_PURE;
        SDL_STDMETHOD(GetEntries)(SDL_THIS_ SDL_DWORD,SDL_DWORD,SDL_DWORD,SDL_LPPALETTEENTRY) SDL_PURE;
SDL_STDMETHOD(Initialize)(SDL_THIS_ LPDIRECTDRAW, SDL_DWORD, SDL_LPPALETTEENTRY) SDL_PURE;
        SDL_STDMETHOD(SetEntries)(SDL_THIS_ SDL_DWORD,SDL_DWORD,SDL_DWORD,SDL_LPPALETTEENTRY) SDL_PURE;
};
#undef SDL_INTERFACE
 ...
 */
#if defined( _WIN32 ) && !defined( _NO_COM )
#define SDL_INTERFACE IDirectDrawClipper
SDL_DECLARE_INTERFACE_( IDirectDrawClipper, SDL_IUnknown )
{
/*** IUnknown methods ***/
SDL_STDMETHOD(QueryInterface) (SDL_THIS_ SDL_REFIID riid, SDL_LPVOID SDL_FAR * ppvObj) SDL_PURE;
SDL_STDMETHOD_(SDL_ULONG,AddRef) (SDL_THIS)  SDL_PURE;
...
SDL_STDMETHOD(GetClipList)(SDL_THIS_ SDL_LPRECT, SDL_LPRGNDATA, SDL_LPDWORD) SDL_PURE;
        SDL_STDMETHOD(GetHWnd)(SDL_THIS_ SDL_HWND SDL_FAR *) SDL_PURE;
SDL_STDMETHOD(Initialize)(SDL_THIS_ LPDIRECTDRAW, SDL_DWORD) SDL_PURE;
        SDL_STDMETHOD(IsClipListChanged)(SDL_THIS_ SDL_BOOL SDL_FAR *) SDL_PURE;
SDL_STDMETHOD(SetClipList)(SDL_THIS_ SDL_LPRGNDATA,SDL_DWORD) SDL_PURE;
        SDL_STDMETHOD(SetHWnd)(SDL_THIS_ SDL_DWORD, SDL_HWND ) SDL_PURE;
};
#undef SDL_INTERFACE
 ...
 */
#if defined( _WIN32 ) && !defined( _NO_COM )
#define SDL_INTERFACE IDirectDrawSurface
SDL_DECLARE_INTERFACE_( IDirectDrawSurface, SDL_IUnknown )
{
/*** IUnknown methods ***/
SDL_STDMETHOD(QueryInterface) (SDL_THIS_ SDL_REFIID riid, SDL_LPVOID SDL_FAR * ppvObj) SDL_PURE;
SDL_STDMETHOD_(SDL_ULONG,AddRef) (SDL_THIS)  SDL_PURE;
...
SDL_STDMETHOD(AddAttachedSurface)(SDL_THIS_ LPDIRECTDRAWSURFACE) SDL_PURE;
        SDL_STDMETHOD(AddOverlayDirtyRect)(SDL_THIS_ SDL_LPRECT) SDL_PURE;
SDL_STDMETHOD(Blt)(SDL_THIS_ SDL_LPRECT,LPDIRECTDRAWSURFACE, SDL_LPRECT,SDL_DWORD, LPDDBLTFX) SDL_PURE;
        SDL_STDMETHOD(BltBatch)(SDL_THIS_ LPDDBLTBATCH, SDL_DWORD, SDL_DWORD ) SDL_PURE;
SDL_STDMETHOD(BltFast)(SDL_THIS_ SDL_DWORD,SDL_DWORD,LPDIRECTDRAWSURFACE, SDL_LPRECT,SDL_DWORD) SDL_PURE;
        SDL_STDMETHOD(DeleteAttachedSurface)(SDL_THIS_ SDL_DWORD,LPDIRECTDRAWSURFACE) SDL_PURE;
SDL_STDMETHOD(EnumAttachedSurfaces)(SDL_THIS_ SDL_LPVOID,LPDDENUMSURFACESCALLBACK) SDL_PURE;
        SDL_STDMETHOD(EnumOverlayZOrders)(SDL_THIS_ SDL_DWORD,SDL_LPVOID,LPDDENUMSURFACESCALLBACK) SDL_PURE;
SDL_STDMETHOD(Flip)(SDL_THIS_ LPDIRECTDRAWSURFACE, SDL_DWORD) SDL_PURE;
        SDL_STDMETHOD(GetAttachedSurface)(SDL_THIS_ LPDDSCAPS, LPDIRECTDRAWSURFACE SDL_FAR *) SDL_PURE;
SDL_STDMETHOD(GetBltStatus)(SDL_THIS_ SDL_DWORD) SDL_PURE;
        SDL_STDMETHOD(GetCaps)(SDL_THIS_ LPDDSCAPS) SDL_PURE;
SDL_STDMETHOD(GetClipper)(SDL_THIS_ LPDIRECTDRAWCLIPPER SDL_FAR*) SDL_PURE;
        SDL_STDMETHOD(GetColorKey)(SDL_THIS_ SDL_DWORD, LPDDCOLORKEY) SDL_PURE;
SDL_STDMETHOD(GetDC)(SDL_THIS_ SDL_HDC SDL_FAR *) SDL_PURE;
        SDL_STDMETHOD(GetFlipStatus)(SDL_THIS_ SDL_DWORD) SDL_PURE;
SDL_STDMETHOD(GetOverlayPosition)(SDL_THIS_ SDL_LPLONG, SDL_LPLONG ) SDL_PURE;
        SDL_STDMETHOD(GetPalette)(SDL_THIS_ LPDIRECTDRAWPALETTE SDL_FAR*) SDL_PURE;
SDL_STDMETHOD(GetPixelFormat)(SDL_THIS_ LPDDPIXELFORMAT) SDL_PURE;
        SDL_STDMETHOD(GetSurfaceDesc)(SDL_THIS_ LPDDSURFACEDESC) SDL_PURE;
SDL_STDMETHOD(Initialize)(SDL_THIS_ LPDIRECTDRAW, LPDDSURFACEDESC) SDL_PURE;
        SDL_STDMETHOD(IsLost)(SDL_THIS) SDL_PURE;
SDL_STDMETHOD(Lock)(SDL_THIS_ SDL_LPRECT,LPDDSURFACEDESC,SDL_DWORD,SDL_HANDLE) SDL_PURE;
        SDL_STDMETHOD(ReleaseDC)(SDL_THIS_ SDL_HDC) SDL_PURE;
SDL_STDMETHOD(Restore)(SDL_THIS) SDL_PURE;
        SDL_STDMETHOD(SetClipper)(SDL_THIS_ LPDIRECTDRAWCLIPPER) SDL_PURE;
SDL_STDMETHOD(SetColorKey)(SDL_THIS_ SDL_DWORD, LPDDCOLORKEY) SDL_PURE;
        SDL_STDMETHOD(SetOverlayPosition)(SDL_THIS_ SDL_LONG, SDL_LONG ) SDL_PURE;
SDL_STDMETHOD(SetPalette)(SDL_THIS_ LPDIRECTDRAWPALETTE) SDL_PURE;
        SDL_STDMETHOD(Unlock)(SDL_THIS_ SDL_LPVOID) SDL_PURE;
SDL_STDMETHOD(UpdateOverlay)(SDL_THIS_ SDL_LPRECT, LPDIRECTDRAWSURFACE,SDL_LPRECT,SDL_DWORD, LPDDOVERLAYFX) SDL_PURE;
        SDL_STDMETHOD(UpdateOverlayDisplay)(SDL_THIS_ SDL_DWORD) SDL_PURE;
SDL_STDMETHOD(UpdateOverlayZOrder)(SDL_THIS_ SDL_DWORD, LPDIRECTDRAWSURFACE) SDL_PURE;
};
#undef SDL_INTERFACE
 ...
 */
#define SDL_INTERFACE IDirectDrawSurface2
SDL_DECLARE_INTERFACE_( IDirectDrawSurface2, SDL_IUnknown )
{
/*** IUnknown methods ***/
SDL_STDMETHOD(QueryInterface) (SDL_THIS_ SDL_REFIID riid, SDL_LPVOID SDL_FAR * ppvObj) SDL_PURE;
SDL_STDMETHOD_(SDL_ULONG,AddRef) (SDL_THIS)  SDL_PURE;
SDL_STDMETHOD_(SDL_ULONG,Release) (SDL_THIS) SDL_PURE;
/*** IDirectDrawSurface methods ***/
SDL_STDMETHOD(AddAttachedSurface)(SDL_THIS_ LPDIRECTDRAWSURFACE2) SDL_PURE;
        SDL_STDMETHOD(AddOverlayDirtyRect)(SDL_THIS_ SDL_LPRECT) SDL_PURE;
SDL_STDMETHOD(Blt)(SDL_THIS_ SDL_LPRECT,LPDIRECTDRAWSURFACE2, SDL_LPRECT,SDL_DWORD, LPDDBLTFX) SDL_PURE;
        SDL_STDMETHOD(BltBatch)(SDL_THIS_ LPDDBLTBATCH, SDL_DWORD, SDL_DWORD ) SDL_PURE;
SDL_STDMETHOD(BltFast)(SDL_THIS_ SDL_DWORD,SDL_DWORD,LPDIRECTDRAWSURFACE2, SDL_LPRECT,SDL_DWORD) SDL_PURE;
        SDL_STDMETHOD(DeleteAttachedSurface)(SDL_THIS_ SDL_DWORD,LPDIRECTDRAWSURFACE2) SDL_PURE;
SDL_STDMETHOD(EnumAttachedSurfaces)(SDL_THIS_ SDL_LPVOID,LPDDENUMSURFACESCALLBACK) SDL_PURE;
        SDL_STDMETHOD(EnumOverlayZOrders)(SDL_THIS_ SDL_DWORD,SDL_LPVOID,LPDDENUMSURFACESCALLBACK) SDL_PURE;
SDL_STDMETHOD(Flip)(SDL_THIS_ LPDIRECTDRAWSURFACE2, SDL_DWORD) SDL_PURE;
        SDL_STDMETHOD(GetAttachedSurface)(SDL_THIS_ LPDDSCAPS, LPDIRECTDRAWSURFACE2 SDL_FAR *) SDL_PURE;
SDL_STDMETHOD(GetBltStatus)(SDL_THIS_ SDL_DWORD) SDL_PURE;
        SDL_STDMETHOD(GetCaps)(SDL_THIS_ LPDDSCAPS) SDL_PURE;
SDL_STDMETHOD(GetClipper)(SDL_THIS_ LPDIRECTDRAWCLIPPER SDL_FAR*) SDL_PURE;
        SDL_STDMETHOD(GetColorKey)(SDL_THIS_ SDL_DWORD, LPDDCOLORKEY) SDL_PURE;
...
SDL_STDMETHOD(SetPalette)(SDL_THIS_ LPDIRECTDRAWPALETTE) SDL_PURE;
        SDL_STDMETHOD(Unlock)(SDL_THIS_ SDL_LPVOID) SDL_PURE;
SDL_STDMETHOD(UpdateOverlay)(SDL_THIS_ SDL_LPRECT, LPDIRECTDRAWSURFACE2,SDL_LPRECT,SDL_DWORD, LPDDOVERLAYFX) SDL_PURE;
        SDL_STDMETHOD(UpdateOverlayDisplay)(SDL_THIS_ SDL_DWORD) SDL_PURE;
SDL_STDMETHOD(UpdateOverlayZOrder)(SDL_THIS_ SDL_DWORD, LPDIRECTDRAWSURFACE2) SDL_PURE;
        /*** Added in the v2 interface ***/
        SDL_STDMETHOD(GetDDInterface)(SDL_THIS_ SDL_LPVOID SDL_FAR *) SDL_PURE;
SDL_STDMETHOD(PageLock)(SDL_THIS_ SDL_DWORD) SDL_PURE;
        SDL_STDMETHOD(PageUnlock)(SDL_THIS_ SDL_DWORD) SDL_PURE;
};
#undef SDL_INTERFACE
 ...
 */
#define SDL_INTERFACE IDirectDrawSurface3
SDL_DECLARE_INTERFACE_( IDirectDrawSurface3, SDL_IUnknown )
{
/*** IUnknown methods ***/
SDL_STDMETHOD(QueryInterface) (SDL_THIS_ SDL_REFIID riid, SDL_LPVOID SDL_FAR * ppvObj) SDL_PURE;
SDL_STDMETHOD_(SDL_ULONG,AddRef) (SDL_THIS)  SDL_PURE;
SDL_STDMETHOD_(SDL_ULONG,Release) (SDL_THIS) SDL_PURE;
/*** IDirectDrawSurface methods ***/
SDL_STDMETHOD(AddAttachedSurface)(SDL_THIS_ LPDIRECTDRAWSURFACE3) SDL_PURE;
        SDL_STDMETHOD(AddOverlayDirtyRect)(SDL_THIS_ SDL_LPRECT) SDL_PURE;
SDL_STDMETHOD(Blt)(SDL_THIS_ SDL_LPRECT,LPDIRECTDRAWSURFACE3, SDL_LPRECT,SDL_DWORD, LPDDBLTFX) SDL_PURE;
        SDL_STDMETHOD(BltBatch)(SDL_THIS_ LPDDBLTBATCH, SDL_DWORD, SDL_DWORD ) SDL_PURE;
SDL_STDMETHOD(BltFast)(SDL_THIS_ SDL_DWORD,SDL_DWORD,LPDIRECTDRAWSURFACE3, SDL_LPRECT,SDL_DWORD) SDL_PURE;
        SDL_STDMETHOD(DeleteAttachedSurface)(SDL_THIS_ SDL_DWORD,LPDIRECTDRAWSURFACE3) SDL_PURE;
SDL_STDMETHOD(EnumAttachedSurfaces)(SDL_THIS_ SDL_LPVOID,LPDDENUMSURFACESCALLBACK) SDL_PURE;
        SDL_STDMETHOD(EnumOverlayZOrders)(SDL_THIS_ SDL_DWORD,SDL_LPVOID,LPDDENUMSURFACESCALLBACK) SDL_PURE;
SDL_STDMETHOD(Flip)(SDL_THIS_ LPDIRECTDRAWSURFACE3, SDL_DWORD) SDL_PURE;
        SDL_STDMETHOD(GetAttachedSurface)(SDL_THIS_ LPDDSCAPS, LPDIRECTDRAWSURFACE3 SDL_FAR *) SDL_PURE;
SDL_STDMETHOD(GetBltStatus)(SDL_THIS_ SDL_DWORD) SDL_PURE;
        SDL_STDMETHOD(GetCaps)(SDL_THIS_ LPDDSCAPS) SDL_PURE;
SDL_STDMETHOD(GetClipper)(SDL_THIS_ LPDIRECTDRAWCLIPPER SDL_FAR*) SDL_PURE;
        SDL_STDMETHOD(GetColorKey)(SDL_THIS_ SDL_DWORD, LPDDCOLORKEY) SDL_PURE;
...
SDL_STDMETHOD(SetPalette)(SDL_THIS_ LPDIRECTDRAWPALETTE) SDL_PURE;
        SDL_STDMETHOD(Unlock)(SDL_THIS_ SDL_LPVOID) SDL_PURE;
SDL_STDMETHOD(UpdateOverlay)(SDL_THIS_ SDL_LPRECT, LPDIRECTDRAWSURFACE3,SDL_LPRECT,SDL_DWORD, LPDDOVERLAYFX) SDL_PURE;
        SDL_STDMETHOD(UpdateOverlayDisplay)(SDL_THIS_ SDL_DWORD) SDL_PURE;
SDL_STDMETHOD(UpdateOverlayZOrder)(SDL_THIS_ SDL_DWORD, LPDIRECTDRAWSURFACE3) SDL_PURE;
        /*** Added in the v2 interface ***/
        SDL_STDMETHOD(GetDDInterface)(SDL_THIS_ SDL_LPVOID SDL_FAR *) SDL_PURE;
SDL_STDMETHOD(PageLock)(SDL_THIS_ SDL_DWORD) SDL_PURE;
        SDL_STDMETHOD(PageUnlock)(SDL_THIS_ SDL_DWORD) SDL_PURE;
/*** Added in the V3 interface ***/
SDL_STDMETHOD(SetSurfaceDesc)(SDL_THIS_ LPDDSURFACEDESC, SDL_DWORD) SDL_PURE;
};
#undef SDL_INTERFACE
 ...
 */
#if defined( _WIN32 ) && !defined( _NO_COM )
#define SDL_INTERFACE IDirectDrawColorControl
SDL_DECLARE_INTERFACE_( IDirectDrawColorControl, SDL_IUnknown )
{
/*** IUnknown methods ***/
SDL_STDMETHOD(QueryInterface) (SDL_THIS_ SDL_REFIID riid, SDL_LPVOID SDL_FAR * ppvObj) SDL_PURE;
SDL_STDMETHOD_(SDL_ULONG,AddRef) (SDL_THIS)  SDL_PURE;
...
SDL_STDMETHOD(GetColorControls)(SDL_THIS_ LPDDCOLORCONTROL) SDL_PURE;
        SDL_STDMETHOD(SetColorControls)(SDL_THIS_ LPDDCOLORCONTROL) SDL_PURE;
};
#undef SDL_INTERFACE
...
{
    SDL_DWORD		dwSize;			// size of the DDSURFACEDESC structure
    SDL_DWORD		dwFlags;		// determines what fields are valid
    SDL_DWORD		dwHeight;		// height of surface to be created
    SDL_DWORD		dwWidth;		// width of input surface
    union
    {
        SDL_LONG		lPitch;			// distance to start of next line (return value only)
        SDL_DWORD           dwLinearSize;           // Formless late-allocated optimized surface size
    };
    SDL_DWORD		dwBackBufferCount;	// number of back buffers requested
    union
    {
        SDL_DWORD           dwMipMapCount;          // number of mip-map levels requested
        SDL_DWORD		dwZBufferBitDepth;	// depth of Z buffer requested
        SDL_DWORD		dwRefreshRate;		// refresh rate (used when display mode is described)
    };
    SDL_DWORD		dwAlphaBitDepth;	// depth of alpha buffer requested
    SDL_DWORD		dwReserved;		// reserved
    SDL_LPVOID		lpSurface;		// pointer to the associated surface memory
...
typedef struct _DDCOLORCONTROL
{
    SDL_DWORD 		dwSize;
    SDL_DWORD		dwFlags;
    SDL_LONG		lBrightness;
    SDL_LONG		lContrast;
    SDL_LONG		lHue;
    SDL_LONG 		lSaturation;
    SDL_LONG		lSharpness;
    SDL_LONG		lGamma;
    SDL_LONG		lColorEnable;
    SDL_DWORD		dwReserved1;
} DDCOLORCONTROL;
 ...
 *
 * The return values from DirectDraw Commands and Surface that return an SDL_HRESULT
...
/*
 * The SDL_GUID passed to DirectDrawCreate is not a valid DirectDraw driver
 * identifier.
 */
...
/*
 * Clipper notification requires an SDL_HWND or
 * no SDL_HWND has previously been set as the CooperativeLevel SDL_HWND.
 */
#define DDERR_NOHWND				MAKE_DDHRESULT( 569 )

/*
 * SDL_HWND used by DirectDraw CooperativeLevel has been subclassed,
 * this prevents DirectDraw from restoring state.
 */
#define DDERR_HWNDSUBCLASSED			MAKE_DDHRESULT( 570 )

/*
 * The CooperativeLevel SDL_HWND has already been set.
